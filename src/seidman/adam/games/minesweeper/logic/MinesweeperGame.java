...
public final class MinesweeperGame {
	...
	/**
	 * Essentially a recursively called method. If you click a blank tile, it should
	 * click all the surrounding tiles.
	 * 
	 * @param i
	 *            Index of the blank tile clicked.
	 */
	public void callBlankClicks(Index i) {
	...
	public void checkForWinning() {
		...
		runGameOverSequence(Variables.WIN_MESSAGE, Variables.WIN_MINE_COLOR);
	}
	 ...
	 * 
	 * @param messageText
	 *            A string of text to show the user after they have completed the
	 *            game. IE: win/lose String.
	 * @param mineColor
	 *            Color that mines should become after the game is over | Note- if
	 *            you pick java.awt.Color.RED and the mine is currently
	 *            java.awt.Color.BLUE, then the mine will become a mix of the two
	 *            colors (new Color(255, 0, 255)).
	 */
	public void runGameOverSequence(String messageText, Color mineColor) {
		_initialIndex = null;
		Variables.update();
			...
			for (Tile j : _tiles[i]) {
				if (j.isMine()) {
					j.setBackground((j.getBackground().equals(Color.BLUE) && mineColor.equals(Color.RED))
							? Variables.LOSE_MINE_COLOR_CORRECT
							: mineColor);
				}
			}
		}
